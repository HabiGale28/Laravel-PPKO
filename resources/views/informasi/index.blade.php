...
<body>
    ...
    <header class="header">
        ...
        <ul class="nav-menu">
            ...
            <!-- MENU AKTIF -->
            <li><a href="{{ route('informasi.index') }}" style="color: #00bcd4; border-bottom: 2px solid #00bcd4;">Informasi</a></li>
    ...
    <div class="container">
        ...
        <div class="pengumuman-list">
            ...
            <div class="pengumuman-card">
                <div class="p-date">
                    <span class="day">{{ \Carbon\Carbon::parse($item->tanggal_publish)->format('d') }}</span>
                    <span class="month">{{ \Carbon\Carbon::parse($item->tanggal_publish)->format('M Y') }}</span>
                </div>
                <div class="p-content">
                    <div class="p-tag">Pengumuman</div>
                    <div class="p-title">{{ $item->judul }}</div>
                    <div class="p-desc">{{ Str::limit(strip_tags($item->konten), 150) }}</div>
        ...
        <div class="event-grid">
            ...
            <div class="event-card">
                <div class="event-img" style="background-image: url('{{ $item->gambar ? asset('uploads/event/'.$item->gambar) : 'https://via.placeholder.com/400x300?text=Event' }}');">
                    <div class="event-status">Upcoming</div>
                </div>
                <div class="event-info">
                    <div class="event-date">📅 {{ \Carbon\Carbon::parse($item->tanggal_mulai)->format('d M Y') }}</div>
                    <div class="event-title">{{ $item->judul }}</div>
                    <div class="event-loc">📍 {{ $item->lokasi ?? 'Online' }}</div>
                </div>
            </div>
            @empty
        ...
        <div class="berita-grid">
            ...
            <div class="berita-card">
                ...
                <div class="berita-body">
                    <div class="berita-meta">{{ \Carbon\Carbon::parse($item->created_at)->format('d F Y') }}</div>
                    <div class="berita-title">
                        <a href="{{ route('informasi.berita.show', $item->slug) }}">{{ $item->judul }}</a>
                    </div>
                    <div class="berita-excerpt">{{ Str::limit(strip_tags($item->konten), 100) }}</div>
