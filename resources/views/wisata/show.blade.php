...
<head>
    ...
    <title>{{ $destinasi->judul }} - Wonderful Indonesia</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .detail-hero {
            ...
            background-image: url('{{ $destinasi->gambar_utama ? asset('uploads/'.$destinasi->gambar_utama) : '' }}');
...
<body>
    ...
    <div class="detail-hero">
        <div class="hero-content">
            <h1>{{ $destinasi->judul }}</h1>
            <div class="location">
                📍 {{ $destinasi->lokasi }}, {{ $destinasi->provinsi }}
            </div>
        </div>
    </div>
    ...
    <div class="main-container">
        ...
        <div class="article-content">
            ...
            <span style="background: #ffc107; padding: 5px 15px; border-radius: 15px; font-size: 14px; font-weight: bold; margin-bottom: 20px; display: inline-block;">
                {{ $destinasi->tipe_pariwisata ?? 'Wisata Umum' }}
            </span>

            <!-- Konten -->
            {!! nl2br(e($destinasi->konten ?? $destinasi->deskripsi)) !!}
        </div>

        <!-- Tombol Kembali -->
        <a href="{{ route('wisata.index') }}" class="back-link">
